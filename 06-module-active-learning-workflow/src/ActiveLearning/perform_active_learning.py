...
def write_auto_annotations(simple_al, sources, predictions, inference_input_s3_ref,byom):
    """
    ...
    auto_annotations = simple_al.autoannotate(predictions, sources,byom)
...
def lambda_handler(event, context):
    ...
    byom=meta_data["BringYourOwnModel"]
    input_total = int(meta_data["counts"]["input_total"])
    # Select maximum of 10 records of the input total for next round of manual labeling.
    max_selections = input_total // 1000
    ...
    meta_data["autoannotations"], auto_annotations = write_auto_annotations(
        simple_al, sources, predictions, inference_input_s3_ref,byom
    )
